...
class Pipe:
    ...
    def __init__(self, x):
        ...
        self.top = 0
        self.bottom = 0
        self.PIPE_TOP = pygame.transform.flip(PIPE_IMG, False, True) # Is the flipped img of the bottom pipe
...
def draw_window(win, birds, pipes, base, score, gen, pipe_ind):
    ...
    score_label = STAT_FONT.render("Alive: " + str(len(birds)), 1, "WHITE")
    ...
    for bird in birds:
        draw_lines = True
        # Draws line from each bird to the top / bottom of the pipes
        if draw_lines:
            try:
                pygame.draw.line(win, (255,0,0), (bird.x+bird.img.get_width()/2, bird.y + bird.img.get_height()/2), (pipes[pipe_ind].x + pipes[pipe_ind].PIPE_TOP.get_width()/2, pipes[pipe_ind].height), 3)
                pygame.draw.line(win, (255,0,0), (bird.x+bird.img.get_width()/2, bird.y + bird.img.get_height()/2), (pipes[pipe_ind].x + pipes[pipe_ind].PIPE_BOTTOM.get_width()/2, pipes[pipe_ind].bottom), 3)
            except:
                pass
        bird.draw(win)
